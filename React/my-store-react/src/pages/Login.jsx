...
export const action = (store) => async ({request}) => {
  ...
  try {
    ...
    const errorMessage = error?.response?.data?.error?.message || 'try again'
...
const Login = () => {
  ...
  return (
    <section className="h-screen grid place-items-center">
      <Form method="post" className="card w-96 p-8 bg-base-100 shadow-lg flex flex-column gap-y-4">
        ...
        <button type="button" className="btn btn-secondary btn-block" onClick={loginGuest}>guest user</button>
